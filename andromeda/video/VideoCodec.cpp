...
void VideoCodec::applySettings(void)
{
	...
	if(ret<0)
		std::cout<<"Get buffer of frame failed."<<getErrMsg(ret)<<std::endl;
	ret=av_frame_make_writable(frame);
	if(ret<0)
		std::cout<<"Make buffer of frame writable failed."<<getErrMsg(ret)<<std::endl;
	is_buf_alloc=true;
	ret=avcodec_open2(codec_context,codec,0);
	if(ret<0)
		std::cout<<"Active encoder context failed."<<getErrMsg(ret)<<std::endl;
}
