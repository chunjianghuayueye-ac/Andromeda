...
def_cls_has_func(render_update)
	...
	namespace app {
		...
		private:
			...
			andromeda::graphics::DoubleFramebuffer doubleFramebuffer;//双缓冲
			//主线程函数，负责事件处理和更新
			//内部使用的初始化函数
			void _initialize()
			{
		...
		public:
			...
			Application(const char* window_title=nullptr,int width=800,int height=600,andromeda::image::ColorRGBA backColor_={0,0,0,0},bool isfullscreen=false,GLFWmonitor* monitor_=glfwGetPrimaryMonitor())
			{
				...
				new (&doubleFramebuffer) andromeda::graphics::DoubleFramebuffer(width,height,backColor_);
				doubleFramebuffer.alloc();
			}
			...
			inline void setBackColor(andromeda::image::ColorRGBA color)
			{
				...
				doubleFramebuffer.setClearColor(color);
			}
		};
	}
}
