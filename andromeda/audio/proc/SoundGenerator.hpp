#ifndef ANM2DR_AUDIO_SOUNDGENERATOR
#define ANM2DR_AUDIO_SOUNDGENERATOR
	...
	namespace audio {
		...
		class SingleSoundRawInfo
		{
			public:
				...
				SingleSoundRawInfo(wave_func* wave_funcs_arr=(wave_func*)memasgn<>(malloc(sizeof(&procfunc::sinwav)),&procfunc::sinwav),int wave_funcs_arr_num=1,double **freq_amp_phrase_infos_arr=nullptr,int *freq_amp_phrase_infos_arr_num=nullptr);
...
inline void SoundGenerator::setBitRate(int br)
{
	bit_rate=br;
}

inline int SoundGenerator::getBitRate(void)
{
	return bit_rate;
}

#endif // ANM2DR_AUDIO_SOUNDGENERATOR
